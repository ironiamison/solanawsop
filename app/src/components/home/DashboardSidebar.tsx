...
const NAV: NavItem[] = [
  ...
  { href: "/profile", label: "Profile", icon: "user", badgeKey: "invites" },
];
...
export default function DashboardSidebar({ balanceRaw = 0 }: { balanceRaw?: number }) {
      ...
      <div className="border-t border-white/[0.06] p-3">
        <div className="px-2 py-3">
          ...
          <p className="mt-1.5 text-2xl font-bold tabular-nums tracking-tight">
            {authenticated && balanceRaw > 0 ? formatTokens(balanceRaw) : "—"}
          </p>
          <p className="mt-0.5 text-[11px] text-zinc-600">
            {authenticated ? "Winnings on-chain" : "Connect to view"}
          </p>
        </div>
      </div>
    </aside>
